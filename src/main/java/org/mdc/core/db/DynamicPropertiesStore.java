...
import org.mdc.core.config.Parameter;
...
public class DynamicPropertiesStore extends MdcStoreWithRevoking<BytesCapsule> {
  ...
  private DynamicPropertiesStore(@Value("properties") String dbName) {
    ...
    try {
      ...
      this.saveAccountUpgradeCost(50_000_000_00L); //节点竞选cost 50MDC
    }
    ...
    try {
      ...
      this.saveCreateAccountFee(0L); // 创建账号消耗0 DATA
    }
    ...
    try {
      ...
      this.saveTransactionFee(100L); // 100DATA/byte
    }

    try {
      this.getAssetIssueFee();
    } catch (IllegalArgumentException e) {
      this.saveAssetIssueFee(100_000_000L);    //发币消耗 10MDC
    }

    try {
      this.getUpdateAccountPermissionFee();
    } catch (IllegalArgumentException e) {
      this.saveUpdateAccountPermissionFee(100_000);  //更新账户权限0.001mdc
    }
