...
# Function: to start, stop and restart java-mdc.
...
JAR_FILE_PATH="./build/libs/java-mdc.jar"
PID_FILE_PATH="java-mdc.pid"
LOG_FILE_PATH="java-mdc.log"
...
case "${1}" in
    start)
        ...
        if [ $? -eq 0 ]; then
            echo "Succeeded to start java-mdc."
        else
            echo "Failed to start java-mdc."
        fi
    ;;
    stop)
        ...
        if [ $? -eq 0 ]; then
            rm ${PID_FILE_PATH}
            echo "Succeeded to stop java-mdc."
        else
            echo "Failed to stop java-mdc."
        fi
    ;;
    restart)
